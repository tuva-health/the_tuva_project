...
select
  ...
  , case when coalesce(all_conditions.NORMALIZED_CODE, all_conditions.NORMALIZED_DESCRIPTION) is not null then 'manual'
         when coalesce(icd10.icd_10_cm,icd10.description,icd9.icd_9_cm,icd9.long_description, snomed_ct.snomed_ct, snomed_ct.description) is not null then 'automatic'
